...
export default function SettingsView() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<SettingsRecord | null>(null);

  useEffect(() => {
    const timer = setTimeout(() => {
      const items: SettingsRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `SETTINGS-${1000 + i}`,
        title: `Settings Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "settings"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
