...
export default function HousingView() {
  const [data, setData] = useState<HousingRecord[]>([]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const items: HousingRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `HOUSING-${1000 + i}`,
        title: `Housing Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "housing"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
