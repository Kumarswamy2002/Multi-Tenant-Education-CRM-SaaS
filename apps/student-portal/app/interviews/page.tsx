...
export default function InterviewsView() {
  const [data, setData] = useState<InterviewsRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("ALL");
  const [page, setPage] = useState(1);
  const [selectedRecord, setSelectedRecord] = useState<InterviewsRecord | null>(null);

  useEffect(() => {
    const timer = setTimeout(() => {
      const items: InterviewsRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `INTERVIEWS-${1000 + i}`,
        title: `Interviews Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "interviews"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
