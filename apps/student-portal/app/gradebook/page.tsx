...
export default function GradebookView() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<GradebookRecord | null>(null);

  useEffect(() => {
    const timer = setTimeout(() => {
      const items: GradebookRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `GRADEBOOK-${1000 + i}`,
        title: `Gradebook Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "gradebook"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
