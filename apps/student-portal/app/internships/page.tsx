...
export interface InternshipsRecord {
  id: string;
  title: string;
  category: string;
...
export default function InternshipsView() {
  const [data, setData] = useState<InternshipsRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("ALL");
  const [page, setPage] = useState(1);
  const [selectedRecord, setSelectedRecord] = useState<InternshipsRecord | null>(null);

  useEffect(() => {
    const timer = setTimeout(() => {
      const items: InternshipsRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `INTERNSHIPS-${1000 + i}`,
        title: `Internships Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "internships"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-white mt-2 tracking-tight">
            Internships Management Console
          </h1>
