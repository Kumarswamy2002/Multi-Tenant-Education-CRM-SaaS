...
export default function AdmissionsView() {
  const [data, setData] = useState<AdmissionsRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("ALL");
  const [page, setPage] = useState(1);
  const [selectedRecord, setSelectedRecord] = useState<AdmissionsRecord | null>(null);

  useEffect(() => {
    const timer = setTimeout(() => {
      const items: AdmissionsRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `ADMISSIONS-${1000 + i}`,
        title: `Admissions Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "admissions"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
