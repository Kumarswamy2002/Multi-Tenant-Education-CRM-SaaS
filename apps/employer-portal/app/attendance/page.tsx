...
export default function AttendanceView() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const items: AttendanceRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `ATTENDANCE-${1000 + i}`,
        title: `Attendance Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "attendance"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
