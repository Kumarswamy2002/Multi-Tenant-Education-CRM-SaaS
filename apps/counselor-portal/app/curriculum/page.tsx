...
export default function CurriculumView() {
  const [data, setData] = useState<CurriculumRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("ALL");
  const [page, setPage] = useState(1);
  const [selectedRecord, setSelectedRecord] = useState<CurriculumRecord | null>(null);

  useEffect(() => {
    const timer = setTimeout(() => {
      const items: CurriculumRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `CURRICULUM-${1000 + i}`,
        title: `Curriculum Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "curriculum"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
