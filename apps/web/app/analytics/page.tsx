...
export default function AnalyticsView() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const items: AnalyticsRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `ANALYTICS-${1000 + i}`,
        title: `Analytics Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "analytics"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
