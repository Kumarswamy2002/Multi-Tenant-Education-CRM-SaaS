...
export default function JobsView() {
  const [data, setData] = useState<JobsRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("ALL");
  const [page, setPage] = useState(1);
  const [selectedRecord, setSelectedRecord] = useState<JobsRecord | null>(null);

  useEffect(() => {
    const timer = setTimeout(() => {
      const items: JobsRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `JOBS-${1000 + i}`,
        title: `Jobs Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "jobs"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
