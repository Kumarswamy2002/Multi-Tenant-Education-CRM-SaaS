...
export default function WorkflowsView() {
  const [data, setData] = useState<WorkflowsRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("ALL");
  const [page, setPage] = useState(1);
  const [selectedRecord, setSelectedRecord] = useState<WorkflowsRecord | null>(null);

  useEffect(() => {
    const timer = setTimeout(() => {
      const items: WorkflowsRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `WORKFLOWS-${1000 + i}`,
        title: `Workflows Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "workflows"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
