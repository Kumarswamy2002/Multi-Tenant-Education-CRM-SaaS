...
export default function InvoicesView() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<InvoicesRecord | null>(null);

  useEffect(() => {
    const timer = setTimeout(() => {
      const items: InvoicesRecord[] = Array.from({ length: 15 }).map((_, i) => ({
        id: `INVOICES-${1000 + i}`,
        title: `Invoices Operational Record ${i + 1}`,
        ...
        tags: ["Multi-Tenant", "Enterprise", "invoices"],
      }));
      setData(items);
      setLoading(false);
    }, 200);
