"""
Counselor workbench endpoints: Lead call logs, Follow-ups, Application decisions
...
router = APIRouter(prefix="/portal-counselor-api", tags=["Portal Counselor Api"])
...
def module_health_check(
    ...
    return APIResponse(
        success=True,
        message="portal_counselor_api router online",
        data={"tenant_id": tenant_id, "status": "healthy", "service": "portal_counselor_api"}
    )
...
def list_records(
    ...
    mock_data = [
        {
            "id": f"portal_counselor_api-id-{i}",
            "tenant_id": tenant_id,
